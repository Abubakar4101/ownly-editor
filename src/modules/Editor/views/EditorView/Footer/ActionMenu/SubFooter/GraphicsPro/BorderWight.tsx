...
function BorderWight() {
  const classes = useStyles();
  const {selectedObjectsConfig, onChangeBorderWight} = useContext(EditorContext);

  console.log('selectedObjectsConfig', selectedObjectsConfig);
  return (
      ...
      <FormControl fullWidth variant="filled" size="small">
        <Box display={'flex'} justifyContent={'space-evenly'} alignItems={'center'}>
          <Select
            labelId="borderWight"
            id="borderWight"
            value={selectedObjectsConfig.borderWidth || WightsOptions[0].value}
            onChange={event => {
              // setSelectedPenWight(event.target.value as number);
              onChangeBorderWight(event.target.value as number);
            }}
            renderValue={value => {
              return (
